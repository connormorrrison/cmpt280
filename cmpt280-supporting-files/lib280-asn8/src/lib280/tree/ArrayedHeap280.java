...
public class ArrayedHeap280<I extends Comparable<? super I>> extends ArrayedBinaryTree280<I> implements Dispenser280<I> {
	...
	public static void main(String[] args) {
		...
		for(int i = 1; i <= 10; i++) {
			H.insert(i);
			if(H.item().compareTo(i) != 0) System.out.println("Expected current item to be " + i + ", got " + H.item());
			if(!H.hasHeapProperty()) System.out.println("Does not have heap property.");
		}
		...
		for(int i = 10; i >= 1; i--) {
			...
			if(i==1) { 
				...
				if(H.item().compareTo(i-1) != 0) System.out.println("Expected current item to be " + (i-1) + ", got " + H.item());
				if(!H.hasHeapProperty()) System.out.println("Does not have heap property.");
			}
		}
